...
impl SoulEntity {
    ...
    pub fn new_with_config(entity_name: String, config: &SoulConfig) -> Self {
        Self {
    ...
    /// Record an emotion type with config limits
    pub fn record_emotion_with_config(&mut self, emotion_type: EmotionType, config: &SoulConfig) -> bool {
        if self.emotions.len() >= config.max_emotions_per_entity && !self.emotions.contains_key(&emotion_type) {
            return false;
        }
        *self.emotions.entry(emotion_type).or_insert(0) += 1;
        self.last_interaction = Utc::now();
        true
    }
    ...
    pub fn boost_trust_with_config(&mut self, config: &SoulConfig) {
        let empathy = self.empathy_score();
    ...
    /// Link a memory to the entity with config limits
    pub fn link_memory_with_config(&mut self, memory_id: Uuid, config: &SoulConfig) -> bool {
